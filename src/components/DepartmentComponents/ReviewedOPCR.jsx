...
function ReviewedOPCR(props) {
    ...
    async function handleApproval(){
                var res = await approveOPCR(props.opcr.id).then(data => data.data.message).catch(error => {
                    console.log(error.response.data.error)
                    Swal.fire({
                        title: "Error",
                        text: error.response.data.error,
    ...
    async function approvalOPCR(){
            Swal.fire({
                title:"Approve",
                text:"By approving this OPCR, you acknowledge that this OPCR can be consolidated for the Master OPCR. Do you wish to proceed?",
                showDenyButton: true,
